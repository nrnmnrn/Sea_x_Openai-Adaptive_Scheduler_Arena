...
來源：[候選包 README](../../build-handoff_final/README.md)、[總 PRD](../../build-handoff_final/PRD.md)。
...
來源：[依賴索引](../../build-handoff_final/sub-PRDs/DEPENDENCIES.md)、[候選 workflow](../../build-handoff_final/workflow.md)。

現有 [backend-contract](../../build-handoff_final/docs/contracts/backend-contract.md) 已定排程、Snapshot、錯誤與部分 adapter 規則。
...
來源：[SP-01](../../build-handoff_final/sub-PRDs/scheduling-arena.md)、[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md)、[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md)、[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md)。
...
來源：[候選 AGENTS](../../build-handoff_final/AGENTS.md)、[依賴索引](../../build-handoff_final/sub-PRDs/DEPENDENCIES.md)、[候選 workflow](../../build-handoff_final/workflow.md)。
既有 [團隊 workflow](../playbook/workflow.md) 只允許前置成果已在 `main`，或取得于喬明確例外同意後承接。
候選包規則不可暗自套用為此既有流程的例外。
來源：[團隊 workflow](../playbook/workflow.md)。
競賽 README 記載題目可於正式開發前調整；候選 README 記載題目更換已獲核准。
根 AGENTS 仍寫產品決策暫停至主辦方回覆題目變更請求。
來源：[競賽 README](../competition/README.md)、[候選包 README](../../build-handoff_final/README.md)、[根 AGENTS.md](../../AGENTS.md)。
...
| --- | --- | --- | --- |
| SP-01 Arena | Scheduler、Snapshot、Job、Skill、Event、metrics、segments、pause/resume seam、UI shell。<br>來源：[SP-01](../../build-handoff_final/sub-PRDs/scheduling-arena.md) | SP-02／03／04。<br>來源：[依賴索引](../../build-handoff_final/sub-PRDs/DEPENDENCIES.md) | 固定 baseline fixture、單一 UI shell binding、Snapshot 取得與修改結果規則；SP-03 依賴表所需的 sandbox 執行能力。 |
| SP-02 Existing adaptation | `TriggerResult`、完整 existing `EvaluationResult[]`、all-skills-failed、active `adaptation_id`、adaptation context。<br>來源：[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md) | SP-03。<br>來源：[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md) | `EvaluationContext`、`ExistingEvaluationSummary` 與全敗完整性規則。 |
| SP-03 Candidate loop | gate-passed Candidate、完整 `EvaluationResult`、不可變 promotion input。<br>來源：[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md) | SP-04。<br>來源：[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md) | Candidate／result 對應、保存與可讀取的不可變證據。 |
| SP-04 Promotion/recovery | register、activate、resume、錯誤恢復與 UI 證據。<br>來源：[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md) | 整體整合。<br>來源：[總 PRD](../../build-handoff_final/PRD.md) | retry/checkpoint 操作語意、resync 後採納規則、各 session 隔離的 evidence。 |
...
來源：[backend-contract：方法表](../../build-handoff_final/docs/contracts/backend-contract.md)。
子 PRD 卻分別由 SP-01／02／03／04 負責 Scheduler／adaptation／gate／promotion。
來源：[總 PRD：子 PRD 責任](../../build-handoff_final/PRD.md)。
...
來源：[SP-01](../../build-handoff_final/sub-PRDs/scheduling-arena.md)、[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md)、[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md)、[backend-contract](../../build-handoff_final/docs/contracts/backend-contract.md)。
若 SP-02 直接等 SP-03 提供共同 gate，會新增依賴索引未列出的反向依賴；目前缺的是共用實作歸屬，不是已證實的程式循環依賴。
來源：[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md)、[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md)。
...
來源：[SP-01](../../build-handoff_final/sub-PRDs/scheduling-arena.md)、[驗收矩陣](../../build-handoff_final/ACCEPTANCE.md)。
...
來源：[總 PRD：自動 adaptation](../../build-handoff_final/PRD.md)、[backend-contract：TriggerResult](../../build-handoff_final/docs/contracts/backend-contract.md)。
factory 的 `initial_jobs` 僅規定完成 `t=0` 到達與 dispatch 後回傳，未足以指定非零 checkpoint 重放。
來源：[backend-contract：共同規則](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
此為條件推論。
來源：[總 PRD：gate 與目前 Policy 重用](../../build-handoff_final/PRD.md)。
P95 空值目前定義為 null；null 對 null 或數值的 gate 比較未定。
不得自行視為 0 或自動通過。
來源：[backend-contract：metrics](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
兩者未定完整交接 schema。
來源：[backend-contract](../../build-handoff_final/docs/contracts/backend-contract.md)、[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md)。
...
來源：[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md)、[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md)。
...
來源：[backend-contract：Skill、Candidate、EvaluationResult](../../build-handoff_final/docs/contracts/backend-contract.md)。

重複 register 依既有規格拒絕；安全 retry 不可藉再 register 實現。
來源：[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md)。

## 缺口 E：pause、recovery 與副作用

現有規格要求 controller 序列化、拒收舊 revision、state-uncertain 分流及禁止自動重送修改操作。
來源：[backend-contract：共同規則與錯誤](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
來源：[總 PRD](../../build-handoff_final/PRD.md)、[backend-contract：共同規則](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
此為依「不重送有副作用操作」提出的建議。
來源：[backend-contract：錯誤](../../build-handoff_final/docs/contracts/backend-contract.md)。

**建議，待核定：**手動 retry 僅重試未完成的外部 stage，不重開版本預算。
五版已確定失敗後能做什麼待產品決策；不得暗中開第六版。
來源：[總 PRD](../../build-handoff_final/PRD.md)。
...
來源：[backend-contract：Snapshot](../../build-handoff_final/docs/contracts/backend-contract.md)、[總 PRD：Demo mode](../../build-handoff_final/PRD.md)。
...
來源：[backend-contract：共同規則與方法表](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
不得直接改既有驗收。
來源：[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md)、[backend-contract：Snapshot](../../build-handoff_final/docs/contracts/backend-contract.md)。

## 缺口 G：外部 Adapter

Planner、Evaluator、Critic 只有名稱與錯誤原則；未定輸入、輸出、schema 校驗、timeout 所有權、sync/async、取消或舊結果丟棄。
來源：[總 PRD](../../build-handoff_final/PRD.md)、[backend-contract：錯誤](../../build-handoff_final/docs/contracts/backend-contract.md)。
...
此回合不新增或修改上述權威文件。
來源：[候選 AGENTS](../../build-handoff_final/AGENTS.md)、[候選 INDEXER](../../build-handoff_final/INDEXER.md)。

## Fixture 矩陣（設計測試，非可執行程式）

| 類別 | 固定／提案場景 | 最小預期 |
| --- | --- | --- |
| 排程 | A–D | 保留既有 Policy、deadline、metrics、不搶占 fixtures。來源：[backend-contract：共同 fixtures](../../build-handoff_final/docs/contracts/backend-contract.md) |
| trigger | 提案：no-trigger／current-reuse／other-reuse／all-failed | 不觸發不評估；目前重用不建 activation；其他重用下次 dispatch 啟用；全敗才交 SP-03。來源：[SP-02](../../build-handoff_final/sub-PRDs/existing-skill-adaptation.md) |
| 評估 | 提案：incomplete-error／candidate-fail-then-pass／five-fails | 不完整不全敗；失敗回饋；第五版停止。來源：[SP-03](../../build-handoff_final/sub-PRDs/candidate-evaluation-loop.md) |
| promotion | 提案：promotion-pass／wrong-run-candidate／duplicate-register | 僅對應且 passed 可註冊；錯 run 與重複均拒絕。來源：[SP-04](../../build-handoff_final/sub-PRDs/skill-promotion-and-recovery.md) |
| recovery | 提案：partial-success-retry／reset-stale-result／two-sessions | 不重送已成功寫入；reset 拒舊結果；session 隔離。來源：[backend-contract：Session 與錯誤](../../build-handoff_final/docs/contracts/backend-contract.md) |
...
來源：[總 PRD：gate](../../build-handoff_final/PRD.md)。

固定結果可證明接線與拒絕規則，不能證明真 gate、sandbox 安全或全隊整合。
正式整合仍依 `main` 證據。
來源：[依賴索引](../../build-handoff_final/sub-PRDs/DEPENDENCIES.md)。
